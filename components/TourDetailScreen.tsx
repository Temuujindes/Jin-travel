...
export default function TourDetailScreen({ slug = 'gobi-4d', tourOverride }: { slug?: string; tourOverride?: Tour }) {
  ...
  const tour = tourOverride || tours.find((item) => item.slug === slug)
  if (!tour) return <main className="app"><Header /><div className="form-page"><h1 className="serif">{t('tourNotFound')}</h1><Link href="/tours" className="primary full">{t('tours')}</Link></div><BottomNav /></main>
  const gallery = tour.gallery.length ? tour.gallery : [tour.image]
  const currentPhoto = Math.min(photo, gallery.length - 1)
