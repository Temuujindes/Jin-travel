...
export default function BookingFlow({ tour, tourId }: { tour: Tour; tourId: string }) {
  ...
  return <main className="app"><Header />{step === 3 ? <div className="form-page success container"><div className="check"><Check size={38} strokeWidth={1.5} /></div><div className="eyebrow">JIN Travel</div><h1 className="serif">{t('bookingComplete')}</h1><p>{t('managerMessage')}</p><div className="reference">{t('reference')}<strong>{referenceCode}</strong></div><Link href="/" className="primary full">{t('home')}</Link></div> : <div className="form-page container"><div className="eyebrow">{t('bookingRequest')}</div><div className="steps"><span className="step current" /><span className={`step ${step >= 2 ? 'current' : ''}`} /><span className="step" /></div>{step === 1 ? <><h1 className="serif">{t('tripDetails')}</h1><p>{t('slower')}</p><div className="booking-fields"><div className="field"><label htmlFor="date">{t('startDate')}</label><input id="date" type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></div><div className="field"><label>{t('travelers')}</label><div className="counter-control"><button aria-label="Remove traveler" onClick={() => setTravelers(Math.max(1, travelers - 1))}><Minus size={15} /></button><strong>{travelers}</strong><button aria-label="Add traveler" onClick={() => setTravelers(travelers + 1)}><Plus size={15} /></button></div></div></div><div className="total-line"><span>{t('bookingRequest')}</span><strong>${tour.priceUsd * travelers}</strong></div><button className="primary full" onClick={() => setStep(2)}>{t('continue')}</button></> : <><h1 className="serif">{t('yourDetails')}</h1><p>{t('managerMessage')}</p><div className="booking-fields"><div className="field"><label htmlFor="name">{t('customerName')}</label><input id="name" value={name} onChange={(event) => setName(event.target.value)} placeholder={t('customerName')} /></div><div className="field"><label htmlFor="contact">{t('kakaoId')}</label><input id="contact" value={contact} onChange={(event) => setContact(event.target.value)} placeholder={t('kakaoId')} /></div><div className="field full-field"><label htmlFor="request">{t('specialRequest')} <span className="muted">{t('optional')}</span></label><textarea id="request" value={specialRequest} onChange={(event) => setSpecialRequest(event.target.value)} placeholder={t('specialRequest')} /></div></div>{error && <p className="error">{error}</p>}<button className="primary full" onClick={submit}>{t('sendRequest')}</button></>}</div>}<BottomNav /></main>
}
