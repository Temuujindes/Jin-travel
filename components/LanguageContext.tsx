...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  const t: Translate = (key, fallback) => { const localized = extra[language][key] || translations[language][key]; const english = translations.en[key as keyof typeof translations.en]; if (!localized && !english && !fallback && process.env.NODE_ENV !== 'production') { const warningKey = `${language}:${key}`; if (!warned.has(warningKey)) { warned.add(warningKey); console.warn(`Missing translation: ${warningKey}`) } } return localized || fallback || english || key }
